...
public class ScoreboardHandler {
    public static void updateScoreboard(TheRoadPlugin plugin){
        ...
        for (Player X : Bukkit.getOnlinePlayers()){
            ...
            Score score;
            if (X.getPersistentDataContainer().has(new NamespacedKey(TheRoadPlugin.getInstance(), "Supporter"), PersistentDataType.BOOLEAN)){
                score = objective.getScore(ChatColor.GOLD+ "⭐" +X.getDisplayName() + ": " + ChatColor.GREEN + plugin.PlayerScores.get(X) + "$");
            }
            else {
                score = objective.getScore(X.getDisplayName() + ": " + ChatColor.GREEN + plugin.PlayerScores.get(X) + "$");
            }
